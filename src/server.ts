...
import { handleTutorChat, getActiveLesson, resetActiveLesson } from './tutor/orchestrator.js';
import { getLLMProvider } from './tutor/llm/provider-adapter.js';
import { synthesizeSpeech, AVAILABLE_VOICES } from './tutor/tts-service.js';
import { searchEducationalImage, searchInternetImage } from './tutor/image-service.js';
import { LIBRARY_REGISTRY, exportExcalidrawLibraryItems } from './tutor/library-registry.js';
...
// ==========================================
// Graphical AI Tutor API Endpoints
// ==========================================

app.post('/api/tutor/chat', async (req: Request, res: Response) => {
  try {
    const result = await handleTutorChat(req.body, (newElements) => {
      const created: ServerElement[] = [];
      newElements.forEach(el => {
        if (el.isDeleted) {
          elements.delete(el.id);
          broadcast({ type: 'element_deleted', elementId: el.id });
        } else {
          elements.set(el.id, el as ServerElement);
          created.push(el as ServerElement);
        }
      });
      if (created.length > 0) {
        ...
        broadcast({
          type: 'elements_batch_created',
          elements: created,
          scrollToElementIds: created.map(e => e.id),
        });
      }
    });
    ...
    logger.error('Error in /api/tutor/chat:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

app.post('/api/tutor/test-connection', async (req: Request, res: Response) => {
  try {
    // Frontend sends { providerConfig: { provider, apiKey, baseUrl, model } }
    const config = req.body.providerConfig || req.body;
    const provider = getLLMProvider(config.provider || 'openai-compatible');
    const result = await provider.validate(config);
    res.json({ success: result.ok, ok: result.ok, error: result.error, message: (result as any).message });
  } catch (error: any) {
    logger.error('Error in /api/tutor/test-connection:', error);
    res.status(500).json({ success: false, ok: false, error: error.message });
  }
});

app.get('/api/tutor/lesson', (req: Request, res: Response) => {
  res.json({ success: true, lesson: getActiveLesson() });
});

app.post('/api/tutor/lesson/reset', (req: Request, res: Response) => {
  const lesson = resetActiveLesson(req.body?.level);
  if (req.body?.clearCanvas) {
    elements.clear();
...
// Returns curated list of high-quality Neural TTS voices (English & Urdu)
app.get('/api/tutor/voices', (_req: Request, res: Response) => {
  res.json({ success: true, voices: AVAILABLE_VOICES });
});

// Synthesizes natural Neural audio for AI tutor speech
app.post('/api/tutor/tts', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Error in /api/tutor/tts:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// Search and retrieve authentic educational diagrams / photos with SSRF protection
app.post('/api/tutor/image-search', async (req: Request, res: Response) => {
  try {
    const { query, preferPinterest } = req.body;
    if (!query || typeof query !== 'string') {
      return res.status(400).json({ success: false, error: 'Query string is required' });
    }
    const image = await searchInternetImage(query, { preferPinterest: !!preferPinterest });
    if (!image) {
      return res.json({ success: false, message: 'No educational image found for query' });
    }
    res.json({ success: true, image });
  } catch (error: any) {
    logger.error('Error in /api/tutor/image-search:', error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// Returns curated vector symbol libraries and formatted Excalidraw library items
app.get('/api/tutor/libraries', (_req: Request, res: Response) => {
  res.json({
    success: true,
    libraries: LIBRARY_REGISTRY,
    libraryItems: exportExcalidrawLibraryItems(),
  });
});


// Cache OpenRouter models for 1 hour to avoid rate limits and provide instant frontend loading
let cachedOpenRouterModels: any[] | null = null;
let lastOpenRouterFetchTime = 0;

app.get('/api/tutor/openrouter/models', async (_req: Request, res: Response) => {
  try {
    const now = Date.now();
    if (cachedOpenRouterModels && (now - lastOpenRouterFetchTime < 3600000)) {
      return res.json({ success: true, count: cachedOpenRouterModels.length, models: cachedOpenRouterModels });
    }
    ...
    const formattedModels = rawList.map((m: any) => {
      ...
      const isZeroCost = !isNaN(promptPrice) && !isNaN(completionPrice) && promptPrice === 0 && completionPrice === 0;
      const isFreeTag = typeof m.id === 'string' && m.id.includes(':free');
      return {
        id: m.id,
        name: m.name || m.id,
        context_length: m.context_length,
        isFree: isZeroCost || isFreeTag,
    ...
    res.json({ success: true, count: formattedModels.length, models: formattedModels });
  } catch (error: any) {
